...
marks that often enclose line names. The rule resolves 2,282 of 2,386 national records
(95.6%) into two parental-line entities; the remaining 4.4% record their origin in prose that
names no explicit cross, and are excluded from the concentration indicator rather than
imputed. We report the extraction rule and its coverage here rather than in §7 because it is
a property of the corpus construction, and because an entity-resolution rule whose failure
rate is not stated is not auditable by anyone re-using the pipeline.
...
so any concentration we report is a lower bound, and the between-channel contrasts in §7 are
...
state this plainly rather than leave it to be inferred. **We have not cross-checked the
...
- **R10 (drop the dominant germplasm lineage).** Dropping every record carrying the single most prevalent sterile-line family leaves the headline result significant with the same sign: head-rice −1.368 pp (p = 0.006, n = 602), chalkiness +0.940 pp (p = 0.034, n = 599), stated grade −0.104 (p = 0.011, n = 609) (Table 4). Conclusion: the result does not depend on any one breeding lineage; see also §7.
...
We resolve the variety-source field into two parental-line entities per record — the female (sterile) and male (restorer) line of the originating cross — which succeeds for 2,282 of 2,386 national records (95.6%; see §3.1 for the extraction rule). Over those entities we compute a germplasm-concentration indicator: the Herfindahl–Hirschman index across sterile lines, the count of distinct lines, and the share held by the single most frequent line. Because concentration measures are non-linear in the sample, differences are assessed by a stratified bootstrap and a label-permutation test, and distinct-line counts are rarefied to the smaller group so they cannot be read as a sample-size artefact.

The two self-organised channels diverge, and they diverge in the direction their institutional design predicts (Table 5; Fig. 6). Green-channel entrants, whose trials are run by a single certified enterprise, draw on a markedly **narrower** base than contemporaneous unified entrants: HHI 0.064 against 0.026 (difference +0.038, 95% CI [+0.009, +0.076], permutation p = 0.009), with the most frequent sterile line covering 19.8% of entries against 8.2%, and 43 distinct sterile lines against a rarefied 58.6. Consortium entrants, whose trials pool five or more breeders, are **more** diverse than the unified trial: HHI 0.0085 against 0.0128 (difference −0.0044, 95% CI [−0.0083, −0.0008], permutation p = 0.016), with 290 distinct lines rarefied against 238. Fig. 6a plots the full concentration curves and Fig. 6b the indices with bootstrap intervals. That the indicator separates the two channels in opposite directions, each matching the number of breeding programmes its rules admit, is evidence that it tracks institutional structure rather than noise — a construct check the channel variable would otherwise lack.

This is what rules the alternative out. A narrow-breeding-base account predicts that the channel drawing on fewer lines shows the quality deficit. Both channels show it — consortium entrants record 1.844 points lower head-rice percentage and 1.108 points higher chalkiness, green-channel entrants 2.809 points higher chalkiness — yet consortium entrants are drawing on a **broader** germplasm base than the state trial they are compared against. Breeding-base narrowness cannot explain a quality deficit in the one channel that is less concentrated than its comparison group. What the two channels share is not their germplasm but their measurement arrangement: in both, the applicant organises the trial in which agronomic performance is recorded, while grain quality is assayed by the same ministry-designated third party either way (§2; §3.3). We claim no more than this: the germplasm-diversity version of the composition story is inconsistent with the data. Other composition stories, including selection on an unobserved entry threshold, remain open and are treated in §4.7 and §6.

Dropping every record carrying the single most prevalent sterile-line family from the main consortium-versus-unified comparison leaves the headline result intact (Section 6.1, R10; Table 4): head-rice percentage −1.368 pp (p = 0.006, n = 602), chalkiness degree +0.940 pp (p = 0.034, n = 599), and stated quality grade −0.104 (p = 0.011, n = 609). The result does not rest on any one breeding lineage.
