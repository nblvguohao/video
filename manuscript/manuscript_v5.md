...
Three implications follow without extending beyond the data. First, the identification argument rests on an asymmetry — third-party assay covers grain quality but not the applicant-measured performance traits — so extending independent measurement to a subset of yield traits, even periodic spot-checks, would let regulators test directly whether the sign separation also appears under independent measurement. Second, an audit-style re-verification of self-organised trial results would generate the within-attribute comparison Duflo et al. (2013) had and this paper does not; such spot-checks are already the Ministry's own initiative since 2022, and this finding is best read as independent quantitative corroboration of that direction, not a claim regulators were unaware of the pattern. Third, the channel-of-entry information already exists inside every announcement but is not published as a standalone field; doing so would let downstream users condition on it directly rather than reconstruct it by parsing.
...
**Administrative approval corpora are a usable, unusually rich innovation data source — within limits.** Innovation measurement has broadened from patents and publications to trademarks, web traces and transaction data (Rammer and Es-Sadki, 2023), but records a state generates when authorising a technology for market have stayed outside the indicator toolkit, chiefly because they are published as prose. They repay parsing: one announcement yields the applicant, testing arrangement, comparison baseline and a dozen-plus performance fields tied to an actual market-entry decision. The limits were equally concrete here — an unverified third-party re-transcription (§3.4), coverage collapsing after 2022, an applicant field absent for four years (§3.3) — so an analyst adopting this class of source should expect most effort to fall on entity resolution and data-quality assessment, not extraction.

**Reliability is a property of fields, not of sources.** Within one document, issued by one authority on one date, fields differed in evidential strength, and what predicted the difference was who measured: laboratory-assayed quality moved one way, applicant-recorded yield gain the other. Standard scientometric data-quality assessment is source-level — coverage, error rates, classification accuracy (Franceschini et al., 2016; Jaffe and de Rassenfosse, 2017) — yet a source that passes such an audit can still contain fields of very different evidential value. Provenance labelling should be a required step: tag each field with its measuring party and stratify reliability assessment by that tag, rather than issuing one verdict for the source.

**The diagnostic is portable.** Applicant-supplied material bound with independent assay results into one official record recurs in drug approval (Shi et al., 2021), device registration, and patent examination, where applicant claims sit beside examiner-added citations (Jaffe and de Rassenfosse, 2017). The test is cheap in each case: partition fields by measuring party and check whether a contrast holds the same sign in both. A contrast present in both partitions is most credible; one confined to the self-supplied partition is a candidate reporting artefact; one confined to the independently assayed partition — this paper's case — is not that kind of artefact, since the reporting party is not the one producing the number, but it is not self-validating either, remaining consistent with selection on an unobserved threshold. It rules out the reporting-incentive explanation and leaves other alternatives to other evidence (§4.7, §6).

**For technology assessment and competitive intelligence.** Text mining of S&T corpora was developed for research management and technology watch (Losiewicz et al., 2000; Antons et al., 2020); approval records extend that practice to the point a technology reaches users. The indicators built here answer which actors produce approved varieties, by which route, at what measured quality, while provenance labels tell the analyst which indicators bear weight. The recommendation for the corpus publisher is cheap: the channel exists inside every announcement already, and issuing it as a field would let downstream users condition on it directly (§8.3).
...
This is a composition effect on the population entering each pathway, not a causal effect on any individual variety. It does not speak to farmer welfare, extension outcomes or seed prices; it does not allege fabrication, a possibility the data cannot distinguish from self-selection or differing thresholds; and it does not extend to every third-party trait or level of government, since disease-resistance grading moves the other way and provincial replication is conclusive for only one of four outcomes. Within these bounds the contribution has two faces. As agricultural technology assessment, it supplies a record-level channel variable, read from announcement text rather than imposed as a before/after indicator, showing that what changed after 2016 was not only volume. As S&T intelligence, it shows a long-public administrative corpus can become a structured indicator system whose fields must be read individually: inside one official source, evidence strength depended on who did the measuring — a diagnostic applicable wherever self-supplied material and independent assay results share an approval record.
